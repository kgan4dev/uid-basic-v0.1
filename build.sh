...
if [ $TARGET_CPU == 'x86' ]; then
	scons WS=off BINDINGS=cpp SERVICES="about,notification,onboarding,controlpanel"
elif [ $TARGET_CPU == 'arm' ]; then
	scons WS=off BINDINGS=cpp CPU=arm OE_BASE=/usr SERVICES="about,notification,onboarding,controlpanel"
fi
...
if [ -f $path/core/alljoyn/core/alljoyn/build/linux/$TARGET_CPU/debug/dist/onboarding/bin/OnboardingClient ]; then
	if [ -f $path/core/alljoyn/core/alljoyn/build/linux/$TARGET_CPU/debug/dist/onboarding/bin/onboarding-daemon ]; then 
		printf "Build successful.!\n"
	else
		printf "\nSomething went wrong.!\n"
		exit
