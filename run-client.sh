...
if [ -z "$arg" ]; then
	printf "USAGE: ./run-client.sh {start/stop/status/restart}\n"
	exit
fi
...
if [ "$arg" == 'start' ]; then
	...
	printf "\nAbout Advertising service 		\n"
	if [ ! -z "$(ps | grep "AboutService" | awk '{print $1}')" ];then
		printf "Service already running. . . \n"
	else
		printf "Service Starting. . .                   "
                $path/core/alljoyn/core/alljoyn/build/linux/$TARGET_CPU/debug/dist/about/bin/AboutService >> $path/logs/alljoyn/AboutService.log 2>&1 &
                printf "Done\n"
	fi
...
#	fi
	...
	if [ ! -z "$(ps | grep "ControlPanelPro" | awk '{print $1}')" ];then
		printf "Service already running. . . \n"
	else
		printf "Service Starting. . .                   "
		$path/core/alljoyn/core/alljoyn/build/linux/$TARGET_CPU/debug/dist/controlpanel/bin/ControlPanelProducer >> $path/logs/alljoyn/ControlPanelProducer.log 2>&1 &
                printf "Done\n"
	fi
...
#	printf "Done\n\n"
	...
	if [ ! -z "$(ps | grep "AboutService" | awk '{print $1}')" ];then
		...
		kill -9 $(ps | grep "AboutService" | awk '{print $1}')
		printf "Done\n"
		printf "Service Starting. . . 			"
		$path/core/alljoyn/core/alljoyn/build/linux/$TARGET_CPU/debug/dist/about/bin/AboutService >> $path/logs/alljoyn/AboutService.log 2>&1 &
		printf "Done\n"
...
#	fi
	...
	if [ ! -z "$(ps | grep "ControlPanelPro" | awk '{print $1}')" ];then
		printf "Service Stopping. . .			"
		kill -9 $(ps | grep "ControlPanelPro" | awk '{print $1}')
		printf "Done\n"
		printf "Service Starting. . . 			"
		$path/core/alljoyn/core/alljoyn/build/linux/$TARGET_CPU/debug/dist/controlpanel/bin/ControlPanelProducer >> $path/logs/alljoyn/ControlPanelProducer.log 2>&1 &
		printf "Done\n"
